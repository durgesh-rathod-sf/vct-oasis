...
import '../vdt/VDTCell.css';
import { ContextMenuTrigger } from 'react-contextmenu';
var SessionStorage = require('store/storages/sessionStorage');

function VDTCellStatic({ page, valueKPI, editNode, parent, onMove, onDrag, onSetName, onKpiNodeClick,
   draggingObject, vdtValues, isGenerateVDTBtnClicked, isBusinessCase, selectedKpiId }) {
  let nodeRef;
  ...
  const newKPI = editNode && editNode.id === valueKPI.id && editNode.creating === true;
  ...
  const renderNewKPI = function () {
    return (
      <Fragment>
        ...
        />
        <datalist id="kpiList" style={{ maxHeight: '20px', overflow: 'scroll' }}>
  ...
  const klass = (parent!== undefined && parent.children.length === 1) ? 'entry sole static-vdt' : 'entry static-vdt';
  ...
  return (<div style={{ opacity }} className={klass}>
    <ContextMenuTrigger id="vdtNodeContextMenu" holdToDisplay={-1} collect={() => { return { type: 'KPI', node: valueKPI } }}>
      <div className={
        ...
        "vdt-kpi-benefit-selected": (valueKPI.id === selectedKpiId)
       ...
       style={{ paddingTop:"12px" }}
        ...
        onClick={() => onKpiNodeClick(valueKPI.id, valueKPI)}
