...
import VDTCellStatic from './VDTCellStatic';
import '../vdt/VDTBranch.css';
var SessionStorage = require('store/storages/sessionStorage');

function VDTBranch({ page, branchTree, level, parent, onMove, onDrag, onAssign, draggingObject, vdtValues, editNode, onSetName,
   onKpiNodeClick, isGenerateVDTBtnClicked, isBusinessCase, selectedKpiId }) {
  const forbidDrag = (SessionStorage.read("demoUser") === "true") || (SessionStorage.read("isMaster") === "Y") || (SessionStorage.read("accessType") === "Read") || isGenerateVDTBtnClicked || isBusinessCase ? false : true;
    ...
    return (
      <Fragment>
        ...
        {branchTree && branchTree.level > 0 && branchTree.level !=2 &&
          <datalist id={`${branchTree.level}_${branchTree.category[0].objType}_list_`} style={{ maxHeight: '20px', overflow: 'scroll' }}>
          ...
          <datalist id={`${branchTree.level}_${branchTree.category[0].objType}_${branchTree.category[0].objCategory}_list_`} style={{ maxHeight: '20px', overflow: 'scroll' }}>
          ...
          <datalist id="so_obj_list" style={{ maxHeight: '20px', overflow: 'scroll' }}>
    ...
    <Fragment>
      {
        ...
        (page === 'projectOverviewDashboard')?
          <VDTCellStatic page={page} valueKPI={branchTree} parent={parent}
           vdtValues={vdtValues}
            editNode={editNode} onSetName={onSetName} onKpiNodeClick={onKpiNodeClick}
          ></VDTCellStatic> : <VDTCellStatic page={page} valueKPI={branchTree} parent={parent}
          onMove={onMove} onDrag={onDrag} draggingObject={draggingObject} vdtValues={vdtValues}
          editNode={editNode} onSetName={onSetName} onKpiNodeClick={onKpiNodeClick} 
          isGenerateVDTBtnClicked={isGenerateVDTBtnClicked} isBusinessCase = {isBusinessCase}
          ...
          <div id={(parent == null) ? 'wrapper' : ''} key={`branch${branchTree.id}`} className={klass} onContextMenu={() => false}>
            {<ContextMenuTrigger id="vdtNodeContextMenu" holdToDisplay={-1} collect={() => { return { type: 'Branch', node: branchTree } }}>
              <span ref={node => drag(drop(node))}  id={getUniqueId(branchTree)}
                className={classNames({ 'isFin': isFinancialClass(branchTree), 'isNonFin': !isFinancialClass(branchTree),'label vdt-node': true, 'droppable': droppable,"ordCursor":( page==='projectOverviewDashboard')}) } style={{ ...opacity,paddingTop:"12px" }}>
                {
            ...
            <div className={`${(branchTree.children.length > 0) ? 'branch lv' + level : ''} ${(!forbidDrag) ? 'branchCursor' : ''}`} onDrop={(e) => console.log('drop ' + e)}>
              {
                branchTree.children && branchTree.children.map((branchSubTree, index1) => (
                  <VDTBranch  page={(page === 'projectOverviewDashboard') ? 'projectOverviewDashboard' : ''} key={branchSubTree.id} level={level + 1} branchTree={branchSubTree} parent={branchTree}
                    ...
                    vdtValues={vdtValues} editNode={editNode} onSetName={onSetName} onKpiNodeClick={onKpiNodeClick} isGenerateVDTBtnClicked={isGenerateVDTBtnClicked}
                    isBusinessCase = {isBusinessCase} selectedKpiId={selectedKpiId}
                  ></VDTBranch>
                ))
              }
            </div>
